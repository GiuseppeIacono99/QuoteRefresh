    ...
    public static String getFinderSurebets() {
        try {
            ...
            try (Response response = client.newCall(request).execute()) {
                ...
                // Prima creiamo un JSONObject del body
                JSONObject json = new JSONObject(body);

                // Poi leggiamo il campo "items" (può essere JSONArray o stringa JSON)
                JSONArray events;
                if (json.get("items") instanceof JSONArray) {
                    events = json.getJSONArray("items");
                } else {
                    String itemsStr = json.getString("items");
                    events = new JSONArray(itemsStr);
                }
                ...
                for (int i = 0; i < events.length(); i++) {
                    ...
                    if (event.has("bookmakers")) {
                        ...
                        for (int j = 0; j < bookmakers.length(); j++) {
                            ...
                            sb.append("🔗 Bookmaker ").append(j + 1).append(": ").append(b.optString("bname")).append("\n");
